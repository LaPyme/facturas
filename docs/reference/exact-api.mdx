---
title: "API exacta"
description: "Controlá el input, la numeración y la evidencia de WSFE, WSMTXCA y Padrón sin derivación de negocio."
keywords: ["API exacta", "WSFE", "WSMTXCA", "Padrón", "numeración"]
---

La API de alto nivel deriva el comprobante por vos. La API exacta te deja armar el
...
- Armá facturas A/B/C desde afirmaciones explícitas con la API de alto nivel.
...
La API de alto nivel también emite por WSMTXCA, con detalle de ítems, cuando pasás
`{ service: "wsmtxca" }`: ver [WSMTXCA](/guides/wsmtxca). Esta API exacta sigue
...
[superficie de alto nivel](/guides/invoices#datos-de-la-factura)) o la
numeración es de tu aplicación, usá la API exacta: `client.wsfe.issue(...)`
...
Para los campos fiscales que la API de alto nivel no expone, usá la salida de emergencia
...
protocolo de ARCA como `PES` y `DOL`; la API y los builders de alto nivel
