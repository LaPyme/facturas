...
certificado. Con `store`, el SDK cifra cada ticket antes de guardarlo, con
AES-256-GCM y una clave derivada de tu clave privada, así que ningún adaptador,
incluido el tuyo, ve el ticket en claro. No hace falta configurar ningún
secreto más: quien tiene la clave privada ya puede iniciar sesión por su cuenta.
Un `wsaaSessionStore` propio recibe las credenciales en claro y el cifrado en
reposo queda de tu lado. Las lecturas del store igual se controlan con el
margen de seguridad de expiración del SDK. Un store de producción tendría que
compartir los datos entre todos los procesos, hacer cumplir la expiración en la
lectura e implementar un bloqueo con `advisory locks` de Postgres, bloqueos de
Redis o un mecanismo equivalente.
...
## Selección avanzada del servicio de emisión

`createArcaClient()` no detecta ni configura el servicio de emisión. Los
métodos de la fachada usan WSFE por defecto. Solo cuando el contribuyente o el
punto de venta opere a través de WSMTXCA, pasá
`{ service: "wsmtxca" }` en la llamada. El SDK no cambia de servicio solo.
Revisá [WSMTXCA para casos requeridos](/guides/wsmtxca).
