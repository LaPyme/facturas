...
| `ArcaInputError` | Input inválido de la aplicación o una clave que `recover()` no puede resolver |
...
- `coe.alreadyAuthenticated`: el SDK deduplica los logins WSAA en vuelo y reusa
  ...
  cola o cualquier despliegue con varios procesos, configurá un
  `wsaaSessionStore` persistente para que un proceso nuevo reuse el TA que obtuvo
  otro. El caché en memoria no se comparte entre procesos.
