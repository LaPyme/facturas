...
`include: { raw: true, sent: true }` agrega la evidencia cruda y el request
enviado a un resultado autorizado, y `NotePreview.originals` lista los
comprobantes que una nota vinculada corrige. Los
[módulos de transporte](/reference/arca-services) documentan `client.wsfe`,
`client.wsmtxca` y `client.padron`.
