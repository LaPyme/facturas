...
keywords: ["createArcaClient", "issue", "preview", "lookup", "lastAuthorized", "recover", "issueCreditNote", "issueDebitNote", "IssueOutcome", "IssuedVoucher"]
---
...
| `lastAuthorized()` | Lee el último número autorizado | [Último número autorizado](#lastauthorized) |
...
```ts
lastAuthorized(
  sequence: { salesPoint: number; voucherType: number },
  options?: PreviewOptions,
): Promise<number>;
```

Devuelve el número del último comprobante que ARCA autorizó para ese punto de
venta y tipo, o `0` si nunca autorizó ninguno. Solo lee: no usa el store ni
reserva número. Con `service: "wsmtxca"` consulta WSMTXCA, y la respuesta es
la misma en los dos servicios.

```ts
const ultimo = await arca.lastAuthorized({ salesPoint: 3, voucherType: 6 });
```
