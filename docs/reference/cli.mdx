...
description: "Opciones, validaciones, diagnósticos, salida JSON y códigos de salida de cada comando de facturas."
keywords: ["CLI", "facturas init", "facturas cert", "facturas check", "facturas issue", "--json"]
---

La referencia completa de `npx facturas`. Si es tu primera vez, empezá por el
[recorrido del CLI](/getting-started/cli), que cuenta en orden qué hace cada
...
misma validación corre en `--cuit` y en `ARCA_TAX_ID` para `check` e
`issue`.
...
- El **CUIT** sale del `serialNumber` del certificado, donde ARCA lo escribe
  ...
  el CLI te pide `--cuit`. Si pasaste uno y el certificado dice otro, el CLI
...
| `--cuit <cuit>` | CUIT, en vez de `ARCA_TAX_ID` |
...
| configuración | `ARCA_TAX_ID` o `--cuit` inválido | CUIT inválido: `<cuit>` tiene `<n>` dígitos y necesita 11. / CUIT inválido: `<cuit>` no pasa el dígito verificador. | Son 11 dígitos y el último es el verificador. Podés escribirlo con guiones. |
| configuración | el certificado no dice el CUIT | El certificado no dice de qué CUIT es. | Pasá `--cuit 20123456786` o definí `ARCA_TAX_ID`. |
...
| certificado y clave | el certificado es de otro CUIT | El certificado es del CUIT `<a>` y el configurado es `<b>`. | Usá el certificado de ese CUIT, o corregí `--cuit` o `ARCA_TAX_ID`. |
