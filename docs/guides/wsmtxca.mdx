...
corresponde, y `amount`. También puede llevar `discount`, `code`, `matrixCode` y
`matrixUnits`.
...
Para llamar a `client.wsmtxca.issue()`, `getLastAuthorizedVoucher()` o
`getVoucher()` sin pasar por `issue()`, mirá
