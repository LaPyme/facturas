---
title: "Facturas"
description: "Emití y previsualizá facturas con la API de alto nivel y manejá resultados fiscales seguros."
keywords: ["facturas", "issue", "preview", "idempotencia", "WSFE"]
---

`issue()` es la API de alto nivel: derivá el comprobante desde datos de negocio, reservá
el número y recuperá los reintentos. Para los campos que no deriva, usá la
[API exacta](/reference/exact-api).
...
[store](/guides/idempotency-stores#postgres) existe.
...
adaptador, está en [Stores](/guides/idempotency-stores#qué-garantiza-cada-store).
...
La API de alto nivel cubre la emisión de CAE de facturas y notas. CAEA, exportación y
