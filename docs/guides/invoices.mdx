...
keywords: ["facturas", "issue", "preview", "idempotencia", "ARCA"]
---
...
pasan tal cual a ARCA. `preview().request` te muestra el comprobante armado
antes de mandarlo.
...
ARCA no recibe una clave de idempotencia. Si la conexión se corta
...
de cualquier I/O con ARCA. Claves distintas identifican operaciones de
negocio distintas.
...
`abortSignal` marca el límite de tiempo con un `AbortSignal`, por ejemplo
...
`recover()` la concilia. No hay un `timeoutMs` aparte: un solo `abortSignal` compone
...
guardada, con el servicio y el número que quedaron registrados: nunca autoriza
...
`forceRefresh`, `include` y `abortSignal`.
...
`voucherType`, la `date` del comprobante en `YYYY-MM-DD`, los mismos `amounts`
que informa un resultado autorizado, y `request`, el pedido que enviaría
`issue()`. El número de comprobante no aparece
porque recién se conoce cuando se reserva al emitir.
...
ARCA solo acepta una fecha cercana al día en que recibe el comprobante, y
`preview()` e `issue()` la controlan antes de enviar nada, contando desde hoy
en Buenos Aires:

| Comprobante | Desde | Hasta |
| --- | --- | --- |
| Productos (concepto 1) | 5 días antes | 5 días después, sin pasar al mes siguiente |
| Servicios (conceptos 2 y 3) | 10 días antes | 10 días después |
| Factura FCE por WSFE | 5 días antes | 1 día después |
| Nota FCE por WSFE | 5 días antes | El límite de su concepto |

Una fecha fuera de esa ventana lanza `ArcaInputError` con `field: "date"`, y el
mensaje dice qué fechas acepta ARCA ese día. Un reintento con la misma
`idempotencyKey` devuelve lo que ARCA ya autorizó aunque la ventana se haya
movido.
...
revisado. Se controla contra las reglas de conciliación de importes de ARCA y
...
`optionalFields`, `buyers` y `activities` pasan tal cual a ARCA. No dupliques
ahí lo que ya informa `fce`.

Cada ítem puede además describir su línea con `description`, `quantity`, `unit`,
`unitPrice`, `discount`, `code`, `matrixCode` y `matrixUnits`. La emisión normal
no usa ese detalle. Si el contribuyente o el punto de venta opera a través de
WSMTXCA, esos campos forman las filas que requiere ese servicio. Revisá el
[caso especial de WSMTXCA](/guides/wsmtxca). El ejemplo compilado de todo esto es
...
una consulta de identidad después de una respuesta indeterminada. Una primera
llamada con clave reserva ese número
...
`conflict` sin ninguna llamada a ARCA.
...
`voucher` habla en las unidades del input: `date` y `caeExpiry` vienen como
`YYYY-MM-DD` y `amounts` en centavos. `voucher.header` trae `concept`, la
identidad y condición del receptor, `currencyId`, `exchangeRate` y, cuando
corresponden, las fechas de servicio y `paymentDueDate`. Las fechas son
`YYYY-MM-DD`, el documento del receptor es un string y el tipo, la condición y
el concepto conservan sus identificadores numéricos de ARCA. `exchangeRate` es
un string decimal exacto. Puede faltar solo cuando ARCA permite omitir la
cotización de una cancelación en la misma moneda extranjera. Las fechas de
servicio faltan para un comprobante de productos y `paymentDueDate` falta
cuando ese comprobante no tiene vencimiento fiscal.

`preview().header` tiene el mismo contrato antes de emitir. Un resultado
`authorized` lo conserva en `voucher.header` tanto si
`recoveredByMatch` es `false` como si es `true`. La consulta y el request siguen
disponibles como evidencia, pero no hace falta ramificar por el modo de
recuperación para guardar la cabecera.

`voucher` trae también `qr`, la URL que tiene que
codificar el código QR del comprobante impreso según la especificación de ARCA,
armada con el CUIT emisor, el total, la moneda, el CAE y el documento del
receptor que se envió de verdad. Falta solo si ARCA devolvió un CAE que la
especificación no puede codificar: una autorización nunca se pierde por su QR. Para reimprimir un comprobante guardado sin
volver a emitir, `arcaQrUrl()` arma la misma URL a partir de esos datos. Solo
`request` conserva la forma de ARCA, con importes en pesos y fechas `YYYYMMDD`.

Si ARCA rechaza el ticket WSAA antes de procesar el pedido, `issue()` pide un
ticket nuevo y reenvía una sola vez. Es seguro porque ese rechazo se clasifica
solo cuando la respuesta no trae CAE, resultado ni número. Con
`forceRefresh: true` no hay segundo intento.

Un `indeterminate` informa en `lookup` por qué quedó abierto: `not_found`,
`incomplete`, `failed`, `aborted` cuando venció tu `abortSignal`, `blocked` cuando
...
El segundo argumento acepta `idempotencyKey`, `abortSignal`, `representedTaxId`,
`forceRefresh`, `service`, `number` e
`include: { rawResponse: true, request: true }`. Los resultados no traen la
respuesta cruda ni el request fiscal por defecto. Si pedís `request`, aparece
en cualquiera de los cuatro resultados y representa lo que el SDK envió o
preparó para ese intento. Una
...
No pases `service` en el uso normal. La emisión usa WSFE por defecto. Usá
`service: "wsmtxca"` solo cuando el contribuyente o el punto de venta opere a
través de ese servicio de ARCA. El SDK no lo descubre ni lo cambia solo. La
[guía de WSMTXCA](/guides/wsmtxca) cubre ese caso especial.

`number` es un número reservado por fuera. Cuando lo pasás, la llamada no lee
el próximo número, así que la aplicación sigue siendo dueña de la secuencia del
punto de venta.
...
servicio de ARCA que quedan fuera de ese conjunto se consideran incompletos.
