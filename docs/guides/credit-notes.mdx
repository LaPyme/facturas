...
description: "Emití notas parciales, totales, por período y FCE con asociaciones fiscales correctas."
keywords: ["nota de crédito", "nota de débito", "FCE", "ARCA"]
---
...
Este bloque es [examples/nota-de-credito-parcial.ts](https://github.com/LaPyme/facturas/blob/main/examples/nota-de-credito-parcial.ts):
...
El SDK consulta el original y toma de ahí la clase, el receptor, la moneda y el
concepto. ARCA no informa la condición de IVA del receptor en comprobantes
autorizados antes de que existiera ese campo. Para esos originales pasá
`to: { condition: "consumidor_final" }` con la condición real del receptor. Se
usa solo si la consulta no la trae, y si la trae tiene que coincidir.
...
Este bloque es [examples/nota-de-credito-total.ts](https://github.com/LaPyme/facturas/blob/main/examples/nota-de-credito-total.ts):
...
fechas de servicio, con el vencimiento elevado a la fecha de la nota. La
aplicación aporta `for`, el modo (`items` o `amounts`, con un `total` opcional,
...
originales van `items` o `amounts`. Repetir las mismas coordenadas en la lista
también se rechaza antes de consultar ARCA.
...
como una nota propia contra su original. Si el contribuyente o el punto de
venta opera a través de WSMTXCA, revisá el
[caso especial de WSMTXCA](/guides/wsmtxca).
...
Una nota FCE sin anulación lleva exactamente una factura FCE asociada. En
WSMTXCA, toda nota FCE lleva exactamente un original, incluso cuando es de
anulación. Esta restricción no afecta las notas ordinarias contra varios
originales.
...
`VoucherSummary` normalizados y sin la respuesta cruda, en el orden en que los
pasaste. Un `VoucherSummary` habla en las unidades del input: importes en
centavos y fechas `YYYY-MM-DD`. Una nota por período no tiene originales, así que tampoco necesita esa
consulta ni devuelve esa propiedad.

`header` describe la nota nueva, no sus originales. Por ejemplo, si la factura
de servicios vencía el 10 de septiembre y la nota se fecha el 19, el SDK ajusta
el vencimiento final de la nota al 19. Ese valor aparece en
`previsualizacion.header.paymentDueDate`.
...
Guardá la misma cabecera después de una autorización directa o recuperada. No
hace falta leer `request`, `lookup` ni `recoveredByMatch`:

```ts
const resultado = await arca.issueCreditNote(input, {
  idempotencyKey: `nc:${devolucion.id}`,
});

if (resultado.kind === "authorized") {
  await guardarComprobante({
    cae: resultado.voucher.cae,
    paymentDueDate: resultado.voucher.header.paymentDueDate,
  });
}

const recuperado = await arca.recover(`nc:${devolucion.id}`);
if (recuperado.kind === "authorized") {
  await guardarComprobante({
    cae: recuperado.voucher.cae,
    paymentDueDate: recuperado.voucher.header.paymentDueDate,
  });
}
```
