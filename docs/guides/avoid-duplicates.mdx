...
ARCA no recibe una clave de idempotencia. Si una respuesta se pierde, no puede
distinguir un reintento de una emisión nueva.
...
Sirve cualquier Postgres, como Supabase, PlanetScale Postgres, Neon o un
servidor propio. Usá el cliente que ya tiene tu aplicación, por ejemplo `pg` o
`postgres`. La función de consulta recibe el SQL y sus parámetros, y puede
devolver una lista de filas o `{ rows }`. Con `postgres`, adaptá
`sql.unsafe(text, params)`. Creá la tabla por defecto una sola vez:
...
```ts
import { createPostgresStore } from "facturas";
import { Pool } from "pg";
...
Las claves usan `arca:v2:wsaa:{environment}:{service}:{fingerprint}` y
`arca:v1:attempt:{environment}:{taxId}:{idempotencyKey}`. El registro de
ticket es un JSON `{ v: 2, iv, data, tag }` cifrado con AES-256-GCM. Un
ticket vigente que una versión anterior dejó en claro bajo `arca:v1:wsaa:` se
vuelve a guardar cifrado en la primera lectura, porque ARCA rechaza un login
nuevo mientras ese ticket vive. La copia en claro queda hasta que vence, para
que los procesos de la versión anterior de un despliegue mixto la sigan
encontrando. El `withLock` de los tickets sigue usando la clave
`arca:v1:wsaa:`, así que las dos versiones serializan sus logins. Los registros de
...
`{ v: 2, kind: "conflict", number, found, settledAt }`, otro comprobante ocupa
el número reservado. `found` es un `VoucherSummary` en centavos y fechas
`YYYY-MM-DD`. Un registro `v: 1` anterior lo guardó en las unidades de ARCA y
se normaliza al leerlo. Una repetición con esa clave o un `recover()` devuelven el
...
Cada registro lleva su versión. `v: 1` es una reserva de WSFE sin detalle.
`v: 2` es una reserva de WSMTXCA o con detalle de ítems, y siempre nombra su
servicio. Un SDK que no conoce `v: 2` no la reproduce, así que volver a una
versión anterior no reenvía por WSFE un comprobante que era de WSMTXCA.
Preservá las dos versiones.
...
operación, otro servicio u otro `number` explícito es una incompatibilidad de
