...
Requiere Node.js 22 o superior y módulos ESM.
...
[Comandos del CLI](/reference/cli#diagnósticos).
...
campos del input están en [Emitir facturas](/guides/invoices#datos-de-la-factura).
...
con `include: { rawResponse: true, request: true }`. La secuencia de llamadas
está en [Qué hace `issue()` al emitir](/guides/invoices#qué-hace-issue-al-emitir).
Cuando una llamada falla, mirá [Errores](/reference/errors).

## 6. Evitá duplicados al reintentar

ARCA no acepta una clave de idempotencia. Si se corta la respuesta,
...
Antes del primer comprobante real, repasá la lista de
[Pasar a producción](/guides/production).
...
en [Notas de crédito](/guides/credit-notes).
